    ...
    <div className="flex-1 min-w-0 flex flex-col">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-[0.15em] text-zinc-500 font-bold">
            {label}
          </span>
        </div>
        {spinning && !locked && (
          <motion.div
            ...
            className="w-3 h-3 border border-rose/30 border-t-rose rounded-full"
          />
        )}
        {locked && (
          <motion.span
            ...
            className="text-[9px] uppercase tracking-wider font-bold text-white bg-rose px-2 py-0.5 rounded-full shadow-sm shadow-rose/30"
          >
            Locked
          </motion.span>
        )}
      ...
      <motion.div
        animate={
          spinning && !locked
            ? {
                borderColor: [
                  "rgba(227,107,138,0.1)",
                  "rgba(227,107,138,0.5)",
                  "rgba(227,107,138,0.1)",
                ],
                boxShadow: [
                  "0 0 0px rgba(227,107,138,0)",
                  "0 4px 30px rgba(227,107,138,0.15)",
                  "0 0 0px rgba(227,107,138,0)",
                ],
              }
            : locked
              ? {
                  borderColor: "rgba(227,107,138,0.35)",
                  boxShadow: "0 4px 25px rgba(227,107,138,0.1)",
                }
              : {
                  borderColor: "rgba(214,188,150,0.25)",
        ...
        <div className="flex-1 flex items-center justify-center relative z-0 px-4">
          <AnimatePresence mode="popLayout">
            {item ? (
              ...
              >
                {proxiedPic ? (
                  ...
                  <motion.span
                    ...
                    transition={locked ? { duration: 0.5 } : {}}
                  ...
                  <div
                    className={cn(
                      "w-24 h-24 sm:w-28 sm:h-28 rounded-full flex items-center justify-center text-4xl font-bold mb-3 transition-all",
                      locked
                        ? "bg-rose text-white shadow-2xl shadow-rose/30"
                ...
                <p
                  className={cn(
                    "font-bold text-lg sm:text-xl truncate max-w-full leading-tight transition-colors",
                    locked ? "text-rose" : "text-zinc-900",
                  )}
                >
                  {item.primary}
                </p>
                {item.secondary && (
                  <p className="text-xs text-zinc-400 max-w-full mt-1 leading-snug line-clamp-2">
                    {item.secondary}
                  </p>
                )}
              </motion.div>
              ...
              >
                <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full bg-blush/30 border-2 border-blush/40 flex items-center justify-center mb-3 shadow-lg shadow-blush/10">
                  <span className="text-5xl text-rose/60">?</span>
                </div>
                <p className="text-blush font-bold text-xl">???</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
...
function SlotLever({
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const rawY = Math.max(
        0,
        Math.min(e.clientY - rect.top - 28, rect.height - 16),
      );
      setDragY(rawY);
    },
    [disabled, fired],
  ...
  return (
    <div className="flex flex-col items-center select-none h-full justify-center">
      ...
      <div
        ref={trackRef}
        className="w-4 bg-gradient-to-b from-beige/80 to-gold/30 rounded-full relative cursor-ns-resize"
        style={{ height: "75%" }}
      >
        <div className="absolute inset-x-0 top-3 bottom-3 bg-beige/30 rounded-full mx-auto w-1" />

        {/* Notch markers */}
        <div className="absolute left-0 right-0 bottom-[30%] flex justify-center">
          <div className="w-2 h-0.5 bg-rose/20 rounded-full" />
        </div>
        <div className="absolute left-0 right-0 bottom-[35%] flex justify-center">
          <div className="w-3 h-0.5 bg-rose/30 rounded-full" />
        </div>
        ...
        <motion.div
          ...
          transition={
            released
              ? {
                  y: {
                    duration: fired ? 1.8 : 0.3,
                    ease: fired ? [0.16, 1, 0.3, 1] : "easeOut",
                  },
                  x: { duration: 0.5 },
                  rotate: { duration: 0.5 },
                }
          ...
          className={cn(
            ...
            isDragging
              ? "bg-rose shadow-rose/40 scale-105"
              : fired
                ? "bg-rose shadow-rose/40"
                : "bg-gradient-to-b from-rose to-rose-dark shadow-rose/20",
      ...
      <p className="text-[8px] text-zinc-400 font-semibold mt-1 uppercase tracking-widest">
        Drag
      </p>
    </div>
  );
        ...
        {players.map((p) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-blush/10 transition-colors"
            >
              <div className="relative shrink-0">
                <div className="w-7 h-7 rounded-full bg-blush/15 flex items-center justify-center text-[10px] font-bold text-rose/50">
...
export default function Roulette({
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const data = await mpFetch<{ players?: RoomPlayer[] }>("room-state", {
  ...
  const profileItems: SlotItem[] = state.profiles.map((p) => ({
    id: p.id,
    primary: `@${p.username}`,
    secondary: p.fullName || undefined,
    pic: p.profilePic || undefined,
  }));
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3">
        <motion.div
          ...
          className="w-10 h-10 border-[3px] border-blush border-t-rose rounded-full"
        />
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
        ...
        <p className="text-sm text-zinc-500">
          Log in or wait for more targets.
        </p>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col items-center px-6 sm:px-10 py-2 overflow-y-auto">
        ...
        >
          <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tighter text-zinc-900">
            {isSpinning ? (
              <span className="font-cursive italic text-rose">
                Choosing fate...
              </span>
            ) : state.phase === "locked" ? (
              <span className="font-cursive italic text-rose">
              ...
              <span>
                Fate has{" "}
                <span className="font-cursive italic text-rose">spoken</span>.
              </span>
            ) : state.phase === "sent" ? (
              <span>
                Shame{" "}
                <span className="font-cursive italic text-rose">delivered</span>
                .
              </span>
            ) : (
              <span>
                Wheel of{" "}
                <span className="font-cursive italic text-rose">Shame</span>
              </span>
            )}
          </h2>
          <p className="text-sm sm:text-base text-zinc-900 mt-1.5 max-w-lg mx-auto leading-relaxed">
            {isSpinning ? (
              <span>
                ...
                <span className="text-rose font-semibold">No take-backs</span>.
              </span>
            ) : state.phase === "locked" ? (
              <span>
                The wheels are{" "}
                <span className="text-rose font-semibold">locking in</span>...
              </span>
            ) : state.phase === "result" || state.phase === "sending" ? (
              <span>
                Review the{" "}
                <span className="text-rose font-semibold">ritual</span> and{" "}
                <span className="text-rose font-semibold">send the shame</span>{" "}
                below.
              </span>
            ) : state.phase === "sent" ? (
              <span>
                The deed is done.{" "}
                <span className="text-rose font-semibold">Spin again</span> for
                more chaos.
              </span>
            ) : isSolo ? (
              <span>
                Spin to pick a{" "}
                <span className="text-rose font-semibold">ritual</span> and a{" "}
                <span className="text-rose font-semibold">target</span>. Pull
                the lever or hit the button.
              </span>
            ) : (
              <span>
                Spin to pick a{" "}
                <span className="text-rose font-semibold">victim</span>, a{" "}
                <span className="text-rose font-semibold">ritual</span>, and a{" "}
                <span className="text-rose font-semibold">target</span>. Pull
                the lever or hit the button.
              </span>
        ...
        <div className="mb-2 max-w-2xl mx-auto w-full">
          {state.phase === "idle" && (
            <motion.button
              ...
              whileHover={{
                scale: 1.02,
                boxShadow: "0 0 50px rgba(227,107,138,0.25)",
              }}
              whileTap={{ scale: 0.97 }}
              onClick={state.spin}
              disabled={!canSpin}
          ...
          {(state.phase === "spinning" || state.phase === "locked") && (
            <motion.p
              ...
              className="text-center text-rose/50 text-sm font-medium py-1"
            >
            ...
            {state.phase === "result" &&
              ...
              state.target && (
                <ResultCard
                  victim={
                    isSolo && selfProfile
                      ? {
                          ...
                          profilePic: selfProfile.profilePic,
                        }
                      : state.victim
                  }
                  ritual={state.ritual}
                  ...
                  onSend={state.executePunishment}
                />
              )}
          </AnimatePresence>

          <AnimatePresence>
            {(state.phase === "sending" || state.phase === "sent") &&
              state.statusText && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className={cn(
                    "w-full text-center py-2.5 px-4 rounded-xl text-xs font-medium mb-1",
                    state.phase === "sent" &&
                      !state.statusText.includes("wrong") &&
                      !state.statusText.includes("failed") &&
                      !state.statusText.includes("Could not")
                      ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
                      : state.phase === "sent"
                        ? "bg-white text-zinc-500 border border-beige/40"
                        : "text-zinc-400",
                  )}
                >
                  {state.phase === "sending" && (
                    <motion.span
                      ...
                      className="inline-block w-3 h-3 border-2 border-beige border-t-rose rounded-full mr-2 align-middle"
                    />
                  )}
                  {state.statusText}
                </motion.div>
